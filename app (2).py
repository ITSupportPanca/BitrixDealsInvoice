...
def create_reset_password_task(email):
    """Buat task di Bitrix24 untuk request reset password."""
    url = WEBHOOK + "tasks.task.add.json"
    params = {
        "fields[TITLE]": f"Reset Password Request — {email}",
        "fields[DESCRIPTION]": f"User {email} melakukan request reset password melalui aplikasi Bitrix24 CRM Report.

Mohon segera reset password dan informasikan ke user yang bersangkutan.",
    ...
    try:
        ...
        data = resp.json()
        if data.get("result"):
            return True, data["result"]["task"]["id"]
        return False, None
                ...
                if not email_reset:
                    ...
                    if email_reset not in users:
                        ...
                        with st.spinner("Mengirim permintaan..."):
                            success, task_id = create_reset_password_task(email_reset)
                        if success:
                            st.success(f"""
✅ **Permintaan reset password berhasil dikirim!**

Tim IT Support telah menerima permintaan Anda dan akan segera menghubungi **{email_reset}** dengan password baru.

📋 Task ID: **#{task_id}**
""")
                        else:
                            st.error(f"❌ Gagal mengirim permintaan. Silakan hubungi IT Support secara langsung.
