...
# ==================== BITRIX TASK ====================
def get_bitrix_user_info(email):
    data = bx_get("user.get.json", {"filter[EMAIL]": email})
...
def create_reset_password_task(email):
    user_id, user_name = get_bitrix_user_info(email)
    from datetime import datetime
    request_time = datetime.now().strftime("%d/%m/%Y %H:%M")
    description = (
        "[B]Request Reset Password[/B]\n\n"
        f"[B]Nama User:[/B] {user_name}\n"
        f"[B]Email:[/B] {email}\n"
        f"[B]Bitrix User ID:[/B] {user_id or 'Tidak ditemukan'}\n"
        f"[B]Waktu Request:[/B] {request_time}\n\n"
        "[B]Tindakan yang diperlukan:[/B]\n"
        "1. Reset password user di Streamlit Secrets\n"
        "2. Informasikan password baru ke user via email/chat\n"
        "3. Close task setelah selesai"
    )
    url = WEBHOOK + "tasks.task.add.json"
    params = {
        "fields[TITLE]": f"[Reset Password] {user_name} ({email})",
        "fields[DESCRIPTION]": description,
        "fields[RESPONSIBLE_ID]": 29,
        "fields[GROUP_ID]": 19,
        "fields[PRIORITY]": 1,
        "fields[TAGS][]": "Reset Password",
    }
    if user_id:
        params["fields[AUDITORS][]"] = user_id
    try:
        resp = SESSION.post(url, params=params, timeout=30)
        resp.raise_for_status()
        data = resp.json()
        if data.get("result"):
            return True, data["result"]["task"]["id"], user_name
        return False, None, user_name
...
def send_otp_bitrix(email, otp):
    """Kirim OTP via Bitrix24 notification."""
    # Cari user ID dari email
    user_id, user_name = get_bitrix_user_info(email)
    if not user_id:
        raise Exception(f"User dengan email {email} tidak ditemukan di Bitrix24.")

    message = (
        f"[B]Kode OTP Login — Bitrix24 CRM Report[/B]\n\n"
        f"Halo {user_name},\n\n"
        f"Kode OTP Anda adalah:\n\n"
        f"[B]{otp}[/B]\n\n"
        f"Kode ini berlaku selama [B]5 menit[/B] dan hanya dapat digunakan sekali.\n\n"
        f"Jika Anda tidak merasa melakukan login, abaikan pesan ini."
    )

    url = WEBHOOK + "im.notify.personal.add.json"
    params = {
        "USER_ID": user_id,
        "MESSAGE": message,
        "MESSAGE_OUT": message,
...
def login_page():
    ...
    if not st.session_state.get("otp_email_sent"):
        st.caption("Masukkan email Anda untuk menerima kode OTP.")
        with st.form("form_email"):
            ...
            if submit:
                if not email:
                    ...
                    otp = generate_otp()
                    try:
                        with st.spinner("Mengirim kode OTP via Bitrix24..."):
                            user_name = send_otp_bitrix(email, otp)
                        st.session_state["otp_code"]       = otp
                        st.session_state["otp_email"]      = email
                        st.session_state["otp_user_name"]  = user_name
                        st.session_state["otp_expired_at"] = datetime.now() + timedelta(minutes=5)
                        st.session_state["otp_email_sent"] = True
                        st.session_state["otp_attempts"]   = 0
                        st.rerun()
                    except Exception as e:
                        st.error(f"❌ Gagal mengirim OTP. Pastikan akun Bitrix24 Anda aktif atau hubungi IT Support.\n\nError: {e}")

    else:
        email      = st.session_state.get("otp_email", "")
        expired_at = st.session_state.get("otp_expired_at")
        ...
        if datetime.now() > expired_at:
            ...
            if st.button("🔄 Minta Kode Baru"):
                for k in ["otp_code","otp_email","otp_expired_at","otp_email_sent","otp_attempts"]:
                    st.session_state.pop(k, None)
                st.rerun()
            return

        sisa  = int((expired_at - datetime.now()).total_seconds())
        menit = sisa // 60
        detik = sisa % 60
